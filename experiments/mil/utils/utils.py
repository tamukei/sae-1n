...
def get_optim(model, cfg_exp):
	if cfg_exp.optimizer.type == "adam":
		optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=cfg_exp.optimizer.lr, weight_decay=cfg_exp.optimizer.reg)
	elif cfg_exp.optimizer.type == 'sgd':
		optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=cfg_exp.oprimizer.lr, momentum=0.9, weight_decay=cfg_exp.optimizer.reg)
	else:
